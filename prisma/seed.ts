  ...
  await prismaClient.$transaction(async (tx: any) => {
    ...
    const combosCategory = await tx.menuCategory.create({
      data: {
        ...
        description: "Combos especiais com acompanhamentos e bebidas",
      },
    });
    ...
    const hamburguersCategory = await tx.menuCategory.create({
      data: {
        name: "Lanches",
        restaurantId: restaurant.id,
        description: "Hambúrgueres e sanduíches deliciosos",
      },
    });
    await tx.product.createMany({
      data: [
    ...
    const frenchFriesCategory = await tx.menuCategory.create({
      data: {
        name: "Fritas",
        restaurantId: restaurant.id,
        description: "Batatas fritas crocantes em diversos tamanhos",
      },
    });
    await tx.product.createMany({
      data: [
    ...
    const desertsCategory = await tx.menuCategory.create({
      data: {
        name: "Sobremesas",
        restaurantId: restaurant.id,
        description: "Sobremesas deliciosas para finalizar sua refeição",
      },
    });
    await tx.product.createMany({
      data: [
